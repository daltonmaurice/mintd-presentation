...
subtitle: "Session 2: Using the Framework with Stata"
author: "Cooper Lab"
format:
  revealjs:
...
* Schema validation
mint_validate_schema using "data/raw/claims.dta", ///
...
```stata
/*
Purpose: Ingest raw Medicare claims data
Input: External data source
Output: data/raw/claims.dta
*/
...
local source = "$MEDICARE_DATA_DIR/claims_2023.csv"
import delimited "`source'", clear

// Save to raw directory
local raw_dir = mint_get_path("raw")
save "`raw_dir'/claims.dta", replace
...
```stata
/*
Purpose: Clean and standardize Medicare claims
Input: data/raw/claims.dta
Output: data/processed/claims_clean.dta
*/
...
// Load raw data
local raw_dir = mint_get_path("raw")
use "`raw_dir'/claims.dta", clear

// Data transformations
drop if missing(claim_id)
rename claim_amt claim_amount
gen claim_year = year(claim_date)

// Validate expected structure
assert !missing(claim_id)
assert claim_amount > 0

// Save processed data
local proc_dir = mint_get_path("processed")
save "`proc_dir'/claims_clean.dta", replace
...
Input: data/processed/claims_clean.dta
...
local proc_dir = mint_get_path("processed")
use "`proc_dir'/claims_clean.dta", clear

// Check for required columns
capture confirm variable claim_id claim_amount claim_year
if _rc != 0 {
...
## Real Example: Medicare Claims Pipeline {.smaller}
...
├── data/
│   ├── raw/claims_*.csv   # Original CMS files
...
1. Install mint if you haven't: `pip install mint`
